document.onload = (function (d3, saveAs, Blob, undefined) {
  ...
  function darSubQueries(node, nodes, edges, queryAcumulada, inicio) {
    if (node.root && !inicio) {
      return [queryAcumulada + node.title];
    } else {
      var subQueries = [];
      if (!inicio) subQueries.push(queryAcumulada + node.title);
      var childrenIds = edges
        .filter((edge) => edge.source == node.id)
        .map((edge) => edge.target); //hijos del hijo

      childrenIds.forEach((id) => {
        var child = nodes.find((d) => d.id == id); //el hijo
        var sqs = darSubQueries(
          child,
          nodes,
          edges,
          queryAcumulada + node.title + "; ",
          false
        );
        sqs.forEach((sq) => {
          subQueries.push(sq);
  ...
  var GraphCreator = function (svg, nodes, edges) {
    ...
    d3.select("#download-input").on("click", function () {
      ...
      //crear querys
      var queries = [];
      const nodes = thisGraph.nodes.filter((nodo) => nodo.root);
      console.log("nodos", nodes);
      nodes.forEach((node) => {
        var subQueries = darSubQueries(
          node,
          thisGraph.nodes,
          saveEdges,
          "",
  ...
  GraphCreator.prototype.changeRootProp = function (d3Node, nodeData) {
    var thisGraph = this;
    if (thisGraph.nodes[thisGraph.nodes.indexOf(nodeData)].root) {
      d3Node.classed(this.consts.rootClass, false);
      thisGraph.nodes[thisGraph.nodes.indexOf(nodeData)].root = false;
    } else {
      d3Node.classed(this.consts.rootClass, true);
      thisGraph.nodes[thisGraph.nodes.indexOf(nodeData)].root = true;
    }
    ...
    if (mouseDownNode !== d) {
      ...
      if (state.justDragged) {
        ...
        if (d3.event.shiftKey) {
          ...
          thisGraph.changeRootProp(d3node, d);
        } else {
  ...
  GraphCreator.prototype.updateGraph = function () {
    ...
    newGs.classed(consts.rootClass, (d) => {
      return d.root;
    });
